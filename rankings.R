...
webpage <- html_nodes(url, '.align-left')
...
#copy <- bba
#split_copy <- str_split(copy, fixed("<"), n = 2, simplify = TRUE)
#split_copy <- unlist(t(str_split(copy, "<", simplify = TRUE)))
# split_copy <- t(str_split(copy, "<", simplify = TRUE))
...
df_new$rank <- round(as.numeric(df_new$rank))

df_new <- df_new %>%
  arrange(rank, bba_rank)
